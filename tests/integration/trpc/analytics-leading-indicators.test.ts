...
describe("analytics.leadingIndicators", () => {
  it("FORBIDDEN when user is not an org member", async () => {
    await expect(
      callTRPCProcedure({
        router: appRouter,
        path: "analytics.leadingIndicators",
  ...
  it("returns null when caller has no observation or CAPA read permission", async () => {
    const out = await callTRPCProcedure({
      router: appRouter,
      path: "analytics.leadingIndicators",
      ctx: ctxWith(createAnalyticsNoPermissionFakeDb(), authenticatedSession()),
      type: "query",
      getRawInput: async () => ({ organizationId: orgId, trailingDays: 30 }),
      signal: testAbortSignal,
      batchIndex: 0,
    });
    expect(out).toBeNull();
